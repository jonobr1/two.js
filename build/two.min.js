...
e)},circle:function(a){var b=parseFloat(a.getAttribute("cx")),f=parseFloat(a.getAttribute("cy")),c=parseFloat(a.getAttribute("r")),e=g.Resolution,n=_.map(_.range(e),function(a){var b=a/e*l;a=c*d(b);b=c*h(b);return new g.Anchor(a,b)},this),n=(new g.Polygon(n,!0,!0)).noStroke();n.translation.set(b,f);return g.Utils.applySvgAttributes(a,n)},ellipse:function(a){var b=parseFloat(a.getAttribute("cx")),f=parseFloat(a.getAttribute("cy")),c=parseFloat(a.getAttribute("rx")),e=parseFloat(a.getAttribute("ry")),
n=g.Resolution,m=_.map(_.range(n),function(a){var b=a/n*l;a=c*d(b);b=e*h(b);return new g.Anchor(a,b)},this),m=(new g.Polygon(m,!0,!0)).noStroke();m.translation.set(b,f);return g.Utils.applySvgAttributes(a,m)},rect:function(a){var b=parseFloat(a.getAttribute("x")),f=parseFloat(a.getAttribute("y")),c=parseFloat(a.getAttribute("width")),d=parseFloat(a.getAttribute("height")),c=c/2,d=d/2,l=[new g.Anchor(c,d),new g.Anchor(-c,d),new g.Anchor(-c,-d),new g.Anchor(c,-d)],l=(new g.Polygon(l,!0)).noStroke();
...
r=g.Utils.Curve.Tolerance,p;u=u||0;if(u>x)return[];var x=(c+f)/2,z=(d+e)/2,F=(f+n)/2,j=(e+m)/2,M=(n+h)/2,N=(m+w)/2,O=(x+F)/2,P=(z+j)/2,F=(F+M)/2,j=(j+N)/2,D=(O+F)/2,E=(P+j)/2;p=h-c;var B=w-d,G=q((f-h)*B-(e-w)*p),H=q((n-h)*B-(m-w)*p);if(0<u){if(G>k&&H>k&&(G+H)*(G+H)<=r.distance*(p*p+B*B)){if(r.angle<r.epsilon)return[new g.Anchor(D,E)];p=b(m-e,n-f);k=q(p-b(e-d,f-c));p=q(b(w-m,h-n)-p);k>=a&&(k=l-k);p>=a&&(p=l-p);if(k+p<r.angle)return[new g.Anchor(D,E)];if(0!==v){if(k>v)return[new g.Anchor(f,e)];if(p>
v)return[new g.Anchor(n,m)]}}}else if(G>k)if(G*G<=r.distance*(p*p+B*B)){if(r.angle<r.epsilon)return[new g.Anchor(D,E)];k=q(b(m-e,n-f)-b(e-d,f-c));k>=a&&(k=l-k);if(k<r.angle)return[new g.Anchor(f,e),new g.Anchor(n,m)];if(0!==v&&k>v)return[new g.Anchor(f,e)]}else if(H>k){if(H*H<=r.distance*(p*p+B*B)){if(r.angle<r.epsilon)return[new g.Anchor(D,E)];k=q(b(w-m,h-n)-b(m-e,n-f));k>=a&&(k=l-k);if(k<r.angle)return[new g.Anchor(f,e),new g.Anchor(n,m)];if(0!==v&&k>v)return[new g.Anchor(n,m)]}}else if(p=D-(c+
h)/2,B=E-(d+w)/2,p*p+B*B<=r.distance)return[new g.Anchor(D,E)];return g.Utils.subdivide(c,d,x,z,O,P,D,E,u+1).concat(g.Utils.subdivide(D,E,F,j,M,N,h,w,u+1))},getCurveFromPoints:function(a,b){for(var c=a.length,d=c-1,l=0;l<c;l++){var e=a[l];_.isObject(e.controls)||g.Anchor.AppendCurveProperties(e);var n=b?z(l-1,c):Math.max(l-1,0),m=b?z(l+1,c):Math.min(l+1,d);r(a[n],e,a[m]);e._command=0===l?g.Commands.move:g.Commands.curve;e.controls.left.x=_.isNumber(e.controls.left.x)?e.controls.left.x:e.x;e.controls.left.y=
