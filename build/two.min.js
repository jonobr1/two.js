...
a.top,c=a.left,f=a.right;a=a.bottom;return new Two.Array([c,b,f,b,c,a,c,a,f,b,f,a])},updateCanvas:function(a){var b=a.commands,c=this.canvas,f=this.ctx,j=a._scale,k=a.stroke,l=a.linewidth*j,q=a.fill,r=a.opacity,v=a.cap,t=a.join,w=a.miter,z=a.curved,e=a.closed,y=b.length,D=y-1;c.width=Math.ceil(a.rect.width*j);c.height=Math.ceil(a.rect.height*j);a=a.rect.centroid;var E=a.x*j,H=a.y*j;f.clearRect(0,0,c.width,c.height);q&&(f.fillStyle=q);k&&(f.strokeStyle=k);l&&(f.lineWidth=l);w&&(f.miterLimit=w);t&&
(f.lineJoin=t);v&&(f.lineCap=v);_.isNumber(r)&&(f.globalAlpha=r);f.beginPath();_.each(b,function(a,c){var k=(a.x*j+E).toFixed(3),m=(a.y*j+H).toFixed(3);if(z){var l=e?p(c-1,y):Math.max(c-1,0),q=e?p(c+1,y):Math.min(c+1,D),x=b[l],q=b[q],l=(x.v.x*j+E).toFixed(3),x=(x.v.y*j+H).toFixed(3),r=(a.u.x*j+E).toFixed(3),n=(a.u.y*j+H).toFixed(3);0>=c?f.moveTo(k,m):(f.bezierCurveTo(l,x,r,n,k,m),c>=D&&e&&(l=(a.v.x*j+E).toFixed(3),x=(a.v.y*j+H).toFixed(3),r=(q.u.x*j+E).toFixed(3),n=(q.u.y*j+H).toFixed(3),k=(q.x*j+
E).toFixed(3),m=(q.y*j+H).toFixed(3),f.bezierCurveTo(l,x,r,n,k,m)))}else 0>=c?f.moveTo(k,m):f.lineTo(k,m)});e&&!z&&f.closePath();f.fill();f.stroke()},updateTexture:function(a,b){this.updateCanvas(b);b.texture&&a.deleteTexture(b.texture);a.bindBuffer(a.ARRAY_BUFFER,b.textureCoordsBuffer);b.texture=a.createTexture();a.bindTexture(a.TEXTURE_2D,b.texture);a.texParameteri(a.TEXTURE_2D,a.TEXTURE_WRAP_S,a.CLAMP_TO_EDGE);a.texParameteri(a.TEXTURE_2D,a.TEXTURE_WRAP_T,a.CLAMP_TO_EDGE);a.texParameteri(a.TEXTURE_2D,
...
fragment:"precision mediump float;\n\nuniform sampler2D u_image;\nvarying vec2 v_textureCoords;\n\nvoid main() {\n  gl_FragColor = texture2D(u_image, v_textureCoords);\n}"}};b.ctx=b.canvas.getContext("2d");var f=Two[Two.Types.webgl]=function(a){this.domElement=document.createElement("canvas");this.elements=[];this.stage=null;a=_.defaults(a||{},{antialias:!1,alpha:!0,premultipliedAlpha:!0,stencil:!0,preserveDrawingBuffer:!1});a=this.ctx=this.domElement.getContext("webgl",a)||this.domElement.getContext("experimental-webgl",
