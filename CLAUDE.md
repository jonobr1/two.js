...
- `npm run types:check` - Type-check all handwritten declarations and the TypeScript usage fixture
...
- TypeScript declaration testing: `npm run types:check`
...
- Check TypeScript declarations with `npm run types:check`
...
- TypeScript declaration tests: Run `npm run types:check`
...
- **TypeScript definitions**: Create a `.d.ts` file alongside each source file (e.g., `src/vector.d.ts` next to `src/vector.js`)
  - Declarations are handwritten and are not generated by `npm run build` or `npm run docs:generate`
  - Update the corresponding `.d.ts` file whenever a public JavaScript API changes
...
- TypeScript tests verify the declarations by compiling the usage fixture and run in CI
- Visual renderer testing remains manual
