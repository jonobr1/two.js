...
* If the patch changes the public API, update the corresponding handwritten `.d.ts` file alongside its JavaScript source file. Add new public exports to `src/two.d.ts` and run `npm run types:check` before opening the pull request. TypeScript declarations are maintained by hand; they are not generated by the build or documentation scripts.
